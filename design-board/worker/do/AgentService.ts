...
export class AgentService {
	...
	constructor(env: Environment) {
		...
		// MiniMax is OpenAI-compatible (chat completions) at api.minimax.io/v1
		this.minimax = createOpenAICompatible({
			name: 'minimax',
	...
	private async *streamActions(prompt: AgentPrompt): AsyncGenerator<Streaming<AgentAction>> {
		...
		// Prefill the start of the actions JSON — only for providers that support
		// response prefill (Anthropic, Google). OpenAI-compatible providers
		// (incl. MiniMax) reject a trailing assistant message, so we skip it there
		// and rely on the system prompt to produce the actions JSON.
		if (provider === 'anthropic.messages' || provider === 'google.generative-ai') {
			messages.push({
				role: 'assistant',
