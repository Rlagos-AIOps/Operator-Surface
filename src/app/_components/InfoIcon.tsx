...
export function InfoIcon({ tooltipKey, text, className = "" }: Props) {
  ...
  return (
    ...
    >
      <Info
        className="h-[14px] w-[14px] text-muted-foreground hover:text-foreground transition-colors duration-fast"
        strokeWidth={1.75}
        aria-hidden
      />
      ...
      <span
        role="tooltip"
        className="pointer-events-none absolute bottom-full left-1/2 z-50 mb-s2 hidden w-[240px] -translate-x-1/2 rounded-md border border-border bg-background px-s3 py-s2 text-micro font-medium leading-snug text-foreground shadow-e2 group-hover/info:block group-focus/info:block"
      >
        {copy}
      </span>
    </span>
