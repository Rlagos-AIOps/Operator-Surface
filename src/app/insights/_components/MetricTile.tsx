...
export function MetricTile({ label, value, eyebrow = "This week", caption }: Props) {
  return (
    <div className="overflow-visible rounded-lg border border-border bg-card p-s5 shadow-e1">
      <p className="font-mono text-micro font-bold uppercase tracking-[0.14em] text-muted-foreground">
        {eyebrow}
      </p>
      <p className="mt-s2 font-serif text-foreground tabular" style={{ fontSize: 56, lineHeight: 1 }}>
        {typeof value === "number" ? value.toLocaleString() : value}
      </p>
      <p className="mt-s3 text-small text-foreground/85">{label}</p>
      {caption && <p className="mt-s1 text-micro text-muted-foreground">{caption}</p>}
    </div>
  );
}
