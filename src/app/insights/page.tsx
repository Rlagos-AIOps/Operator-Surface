...
export const metadata = {
  title: "Insights — Galileo this week",
};
...
export default async function InsightsPage() {
  ...
  const [emailApprovals, runsLastWeek, decisionsLastWeek] = await Promise.all([
    sb
      ...
      // Main accepts both legacy send_email and the canonical send_reply.
      .in("action_type", ["send_email", "send_reply"])
      .eq("status", "approved")
    ...
    <main className="min-h-screen bg-background">
      <div className="mx-auto max-w-[1200px] px-s5 py-s7">
        ...
        <header className="mb-s7">
          ...
          <h1
            className="font-serif text-foreground text-balance mb-s4 max-w-[24ch]"
            style={{
          ...
          <p className="max-w-[60ch] text-body text-foreground/80 leading-relaxed">
        ...
        <p className="mt-s7 max-w-[60ch] text-small text-muted-foreground">
          Window: last 7 days. Fed live from{" "}
          <span className="font-mono text-foreground">agent_runs</span>,{" "}
          <span className="font-mono text-foreground">decisions</span>, and{" "}
          <span className="font-mono text-foreground">approvals</span> — no manual
          tracking.
        </p>
      </div>
    </main>
